...
pts <- read_rds("Data/Rdata/all_pt_attributes.Rdata")
# other visitor groups
co_pts <- pts %>% 
  filter(!is.na(shift_num))
# participants
pt_sample <- pts %>% 
  filter(!is.na(sid))
# staff
staff <- read_rds("Data/staff_attributes.Rdata") %>%
  mutate(node_type = if_else(staff_title == "STAFF", "Staff", staff_title), .keep = "unused")
# RFID data
edges <- read_rds("edges_final_all.Rdata")
```
...
```{r demographics-tbl}
sumtbl_df <- pts %>% mutate(pop_group = "Visitors during study year") %>% 
  bind_rows(co_pts) %>% 
  mutate(pop_group = if_else(is.na(pop_group), "Visitors during observed shifts", pop_group)) %>% 
  bind_rows(pt_sample) %>% 
  mutate(pop_group = if_else(is.na(pop_group), "Visitor sample", pop_group)) %>% 
  mutate(pop_group = factor(pop_group, 
                            levels = c("Visitors during study year", 
                                       "Visitors during observed shifts",
                                       "Visitor sample")))
...
```{r race-group-tble}
sumtbl_df %>%
  filter(pop_group == "Visitor sample" &
           Race != "Other" &
           !is.na(Race)) %>% 
