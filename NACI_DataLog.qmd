...
rmarkdown::paged_table(head(id_sid_clean))
...
rmarkdown::paged_table(head(ids_final))
# Check if sids repeate within shifts:
ids_final %>% 
  group_by(shift_num) %>% 
  count(sid) %>% 
  filter(n > 1) %>% 
  kable(caption = "Are any sids repeated within the same shift?")
...
rmarkdown::paged_table(head(pt_sid_clean))
pt_sid_clean %>% 
  group_by(shift_num) %>% 
  count(shift_ampm) %>% 
  arrange(shift_num) %>% 
  kable(caption = "Patient participants by shift number and day vs. night")

pt_sid_clean %>% 
  count(shift_ampm) %>% 
  kable(caption = "Total patients sampled from day shifts vs. night shifts")
# Save to file
...
staff_sids_clean <- staff_sids %>% 
  ...
  select(-job)
rmarkdown::paged_table(head(staff_sids_clean))
staff_sids_clean %>%
  count(ParticipantCat4) %>% 
  kable(caption = "Staff participants by category")
# Save to file
...
rmarkdown::paged_table(head(allstaff_clean))
...
rmarkdown::paged_table(head(ntab2_clean))
# save to file
...
# shift_times %>% 
#   str()
rmarkdown::paged_table(head(shift_times))
...
rmarkdown::paged_table(head(countspershift_clean))
...
person_dataset <- person_dataset %>% 
  transmute(shift_num = as.integer(shiftnum),
            ...
            degree,
            minutes)
rmarkdown::paged_table(head(person_dataset))
...
rmarkdown::paged_table(head(location_cats))
```
### `location_defs`

```{r location_defs}
location_defs <- read_sas(paste(data_path, "rooms", "location_definitions.sas7bdat", sep = "/"))
rmarkdown::paged_table(head(location_defs))
...
```{r room_locs}
room_locs <- read_xlsx(paste(data_path, "rooms", "Room Locations and Square Footage - with Correction.xlsx", sep = "/"), col_names = c("id", "name", "type", "sqft_area"), col_types = c("text", "text", "text", "numeric", "skip"), skip = 1)
rmarkdown::paged_table(head(room_locs))
```
### `locations_list`

```{r locations_list}
locations_list <- read_sas(paste(data_path, "rooms", "locationslist.sas7bdat", sep = "/"))
rmarkdown::paged_table(head(locations_list))
```
...
rmarkdown::paged_table(head(demographics_clean))
demographics_clean %>% 
  count(race) %>% 
  kable(caption = "Patient race")
demographics_clean %>% 
  group_by(acuity) %>% 
  summarise(`Total (n)` = n(), 
  ...
  kable(caption = "Patient race by ESI acuity")
...
rmarkdown::paged_table(head(pop_pt_demo_clean))
...
rmarkdown::paged_table(head(pts_population))
...
rmarkdown::paged_table(head(pts_pop2))
...
# 3. Delete the prefix "floc" from `seconds` and change class to numeric
  ...
  # glimpse()
# Save as .Rdata file
rmarkdown::paged_table(head(pt_complete_long))
...
rmarkdown::paged_table(head(staff_complete_long))
```
...
rmarkdown::paged_table(head(dictionary_tbl))
```
