...
# write_rds(edges, "edge_seconds-04192023.Rdata") [saved then deleted 04/19/23]
# edges <- read_rds("edges_04192023.Rdata")
```



```{r locations-and-dyads}
...
edges_final_all <- edges %>% left_join(locations, by = c("location_num" = "loc_id")) %>% 
  ...
  mutate(dyad_id = paste(node_id_i, node_id_j, sep = "/"),
    interaction_id = paste(dyad_id, location_num, sep = "-")) ; big_beep(edges_final_all)

# write_rds(edges_final_all, "edges_final_all-041923.Rdata"); big_beep(edges_final_all) [saved 4/19/2023]
```


```{r CI-breaks}
interactions_count <- edges_final_all %>% 
  count(interaction_id); beep(8)

edge_file <- edges_final_all %>% 
  left_join(interactions_count, by = "interaction_id") %>% 
  rename(ci_seconds = n) %>% 
  distinct(interaction_id, .keep_all = TRUE) %>% 
  select(-datetime, -shift_second)

edge_file %>% count(shift_num) %>%
  summarise(ci_mean = mean(n), 
            ci_min = min(n), 
            ci_max = max(n), 
            ci_sd = sd(n),
            ci_median = median(n))
...
big_beep(edge_file)
write_rds(edge_file, "edge_file-04192023.Rdata"); beep(2)
```
...
```{r ci-summarise, include = FALSE, eval=FALSE}
...
```{r race-ci-stats-tbl, eval = FALSE}
...
```{r}
pts_raw <- read_rds("Data/Rdata/all_pt_attributes.Rdata")
...
```{r all-pts}
all_pts <- read_rds("all_pts.Rdata")
all_pts <- all_pts %>% 
  rename(disposition = Disposition,
         ...
         age = Age)
all_pts %>% glimpse()
# write_rds(all_pts, "all_pts.Rdata")
```
